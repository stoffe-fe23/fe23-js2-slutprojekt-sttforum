...
userAPI.post("/profile/update", isLoggedIn, userPictureUpload.single('picture'), (req, res) => {
    try {
        if (req.user && req.user.id) {
            if (!req.body.password.length || (req.body.password && req.body['password-confirm'] && (req.body.password.length > 3) && (req.body.password == req.body['password-confirm']))) {
                const userObj = dataStorage.editUser(req.user.id, req.body.username, req.body.password, req.body.email, req.file.filename);
